...
def sentence1() -> Expr:
    ...
    return logic.conjoin(a_or_b, not_a_if_and_only_if_not_b_or_c, not_a_or_not_b_or_c)
...
def sentence2() -> Expr:
    ...
    not_b_and_not_c_implies_a = (~(B & (~C))) >> A
    not_d_implies_c = ~D >> C

    return logic.conjoin(c_if_and_only_if_b_or_d, a_implies_not_b_and_not_d, not_b_and_not_c_implies_a, not_d_implies_c)
...
def sentence3() -> Expr:
    ...
    pacman_alive = b % ((a & ~d) | (~a & c))
    pacman_contradition = ~(a & c)
    pacman_born = c

    return logic.conjoin(pacman_alive, pacman_contradition, pacman_born)
...
def entails(premise: Expr, conclusion: Expr) -> bool:
    ...
    """
    "*** BEGIN YOUR CODE HERE ***"
    test_expr = premise & ~conclusion
    model = findModel(test_expr)
...
def atMostOne(literals: List[Expr]) -> Expr:
    ...
    """
    "*** BEGIN YOUR CODE HERE ***"
    literal_combinations = itertools.combinations(literals, 2)
    
    clauses = []
    for lit1, lit2 in literal_combinations:
        clause = ~lit1 | ~lit2
        clauses.append(clause)
    
    # Conjoin all the clauses to get the final CNF expression
    if not clauses:  # Handle the case where literals is empty or has one literal
        return True
    elif len(clauses) == 1:
        return clauses[0]
    else:
...
def pacmanSuccessorAxiomSingle(x: int, y: int, time: int, walls_grid: List[List[bool]]=None) -> Expr:
    ...
    "*** BEGIN YOUR CODE HERE ***"
    current_position = PropSymbolExpr(pacman_str, x, y, time=now)
    all_causes_expression = atLeastOne(possible_causes)

    return  conjoin(current_position % all_causes_expression)
...
def pacphysicsAxioms(t: int, all_coords: List[Tuple], non_outer_wall_coords: List[Tuple], walls_grid: List[List] = None, sensorModel: Callable = None, successorAxioms: Callable = None) -> Expr:
    ...
    "*** BEGIN YOUR CODE HERE ***"
    # Walls: If a wall is at (x, y), then Pacman is not at (x, y) at t.
    for x, y in all_coords:
        wall_expr = PropSymbolExpr(wall_str, x, y)  
        pacman_expr = PropSymbolExpr(pacman_str, x, y, time=t)
        implication = wall_expr >> ~pacman_expr
        pacphysics_sentences.append(implication)
    ...
    if sensorModel is not None:
        sensor_axiom = sensorModel(t, non_outer_wall_coords)
        pacphysics_sentences.append(sensor_axiom)
    
    if successorAxioms is not None and t > 0:
        transition_axiom = successorAxioms(t, walls_grid, non_outer_wall_coords)
        pacphysics_sentences.append(transition_axiom)
...
def checkLocationSatisfiability(x1_y1: Tuple[int, int], x0_y0: Tuple[int, int], action0, action1, problem):
    ...
    model1 = findModel(KB&PropSymbolExpr(pacman_str, x1_y1[0], x1_y1[1], time=1))

    model2 = findModel(KB&~PropSymbolExpr(pacman_str, x1_y1[0], x1_y1[1], time=1))
...
def positionLogicPlan(problem) -> List:
    ...
    for t in range(50):
        ...
        if t > 0:
            KB.append(exactlyOne([PropSymbolExpr(action, time=t-1) for action in actions]))
...
def foodLogicPlan(problem) -> List:
    ...
    for t in range(50):
        ...
        goal_assertion = [~PropSymbolExpr(food_str, x, y, time = t) for x,y in food]
        model = findModel(logic.conjoin(goal_assertion + KB))
        if model:
            return extractActionSequence(model, actions)
        ...
        for x, y in food:
            ...
            get_food = food_t & pacman_t
            avoid_food = food_t & ~pacman_t
...
def updateKnowledgeBase(agent, knowledgeBase, timeStep, all_coords, non_outer_wall_coords, walls_grid):
    pacphysics = pacphysicsAxioms(timeStep, all_coords, non_outer_wall_coords, walls_grid, sensorAxioms, allLegalSuccessorAxioms)
    knowledgeBase.append(pacphysics)

    actionExpression = PropSymbolExpr(agent.actions[timeStep], time=timeStep)
    knowledgeBase.append(actionExpression)

    perceptRules = fourBitPerceptRules(timeStep, agent.getPercepts())
    knowledgeBase.append(perceptRules)

def findPossibleLocation(knowledgeBase, timeStep, non_outer_wall_coords, possible_locations):
    combinedKB = logic.conjoin(knowledgeBase)
    x, y = non_outer_wall_coords
    pacman_at_xy = PropSymbolExpr(pacman_str, x, y, time=timeStep)

    if (findModel(combinedKB & pacman_at_xy)):
        possible_locations.append((x, y))

    elif (entails(combinedKB, pacman_at_xy)):
        knowledgeBase.append(pacman_at_xy)

    else:
        knowledgeBase.append(~pacman_at_xy)

def findProvableWallLocation(knowledgeBase, non_outer_wall_coords, known_map):
    combinedKB = logic.conjoin(knowledgeBase)

    x, y = non_outer_wall_coords
    wall_exists = logic.PropSymbolExpr(wall_str, x, y)
    
    # Add to KB and update known_map: (x, y) locations where there is provably a wall.
    if entails(combinedKB, wall_exists):                
        knowledgeBase.append(wall_exists)
        known_map[x][y] = 1
    
    # Add to KB and update known_map: (x, y) locations where there is provably not a wall.
    elif entails(combinedKB, ~wall_exists):
        knowledgeBase.append(~wall_exists)
        known_map[x][y] = 0
...
def localization(problem, agent) -> Generator:
    ...
    for t in range(agent.num_timesteps):
        updateKnowledgeBase(agent, KB, t, all_coords, non_outer_wall_coords, walls_grid)

        possible_locations = []
        for loc in non_outer_wall_coords:
            findPossibleLocation(KB, t, loc, possible_locations)
...
def mapping(problem, agent) -> Generator:
    ...
    for t in range(agent.num_timesteps):
        updateKnowledgeBase(agent, KB, t, all_coords, non_outer_wall_coords, known_map)
        
        for wall in non_outer_wall_coords:
            findProvableWallLocation(KB, wall, known_map)
